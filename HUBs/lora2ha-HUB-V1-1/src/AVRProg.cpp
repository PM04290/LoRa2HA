...
int AVRProg::HEXfileToImage(const char* filename)
{
	uint16_t len;
	uint8_t b, cksum = 0;
	uint16_t lineaddr;
	...
	while(f.available()) {
		...
		len = hexToByte(f.read());
		len = (len << 4) + hexToByte(f.read());
		cksum = len;

		// read high address byte
		b = hexToByte(f.read());
		b = (b << 4) + hexToByte(f.read());
		cksum += b;
		lineaddr = b;
		...
		for (byte i = 0; i < len; i++) {
			...
			if (lineaddr + i < sizeof(FlashMemory)) {
				FlashMemory[lineaddr + i] = b;
				retCode += 1;
			}
		}
		// read checksum
...
bool AVRProg::flashImage(uint16_t  pagesize, uint16_t  codesize)
{
	uint32_t pageaddr = 0;
	uint32_t flash_start = g_updi.config->flash_start;

	while (pageaddr < codesize)
	{
		bool blankpage = true;
		for (uint16_t i = 0; i < pagesize && blankpage; i++)
		{
			if (FlashMemory[pageaddr + i] != 0xFF)
				blankpage = false;
		}
		if (!blankpage)
		{
			byte* imagePage = FlashMemory + pageaddr;
			if (!flashPage(imagePage, flash_start + pageaddr, pagesize))
				return false;
		}
		pageaddr += pagesize;
	}
