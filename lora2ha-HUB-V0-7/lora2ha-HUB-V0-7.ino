...
void processLoRa()
{
  ...
  if (getPacket(&p))
  {
    ...
    {
      if (cp->senderID == RL_ID_PING) // if senderID = 254, ping needed
      {
      ...
      if (ch != nullptr)
      {
        ...
        { // config packet
          ...
          int c;
          String js;
          dev = hub.getDeviceById(cp->senderID);
          ...
          { // Basic configuration
            ...
            DEBUGf("Conf B %d %d %s %d %d\n", cp->senderID, cnfb->childID, cnfb->name, (int)st, (int)dt);
            if (dev == nullptr)
            {
              d = hub.getNbDevice(); // before adding (1st is 0)
              if (cnfb->childID == RL_ID_CONFIG) {
                dev = hub.addDevice(new Device(cp->senderID, cnfb->name, false));
              } else {
                dev = hub.addDevice(new Device(cp->senderID, "", false));
              }
              if (!dev) {
            ...
            if (cnfb->childID != RL_ID_CONFIG)
            {
              ...
              if (ch == nullptr)
              {
                ...
                ch = new Child(dev, cnfb->childID, cName, st, dt, "", CategoryAuto, "");
                if (!ch) {
            ...
            ch = dev->getChildById(cp->data.configs.base.childID);
          }
          DEBUG((long)dev); DEBUG(" "); DEBUGln((long)ch);
  ...
  if (devices.is<JsonArray>())
  {
    for (JsonVariant deviceItem : devices.as<JsonArray>())
    {
      if (!deviceItem.isNull())
      {
        ...
        DEBUGf("@ %d : %s\n", address, name);
        if ((address > 0) && (dev = hub.addDevice(new Device(address, name, true, rlversion))))
        {
          ...
          if (childs.is<JsonArray>())
          {
            for (JsonVariant childItem : childs.as<JsonArray>())
            {
              ...
              if (st == E_SELECT)
              {
                ch->setSelectOptions(childItem["options"].as<const char*>());
              }
              if (st == E_INPUTNUMBER)
              {
