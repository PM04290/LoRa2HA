...
void setup()
{
...
#ifndef NO_UPDI
  avrprog.setUPDI(&Serial2, 115200, SCL, SDA); // (RX, TX) can also try 230400
#endif
  ...
  //             1 = 100m/186 ms
  //             0 = 50m/27 ms
  if (RLcomm.begin(RadioFreq * 1E6, onLoRaReceive, NULL, 20, RadioRange))
  {
...
void loop()
{
  ...
  if (needUploadFilename != "")
  {
    ...
    if (avrprog.targetPower(true))
    {
      ...
      if (signature != 0 && signature != 0xFFFF)
      {
        ...
        if (avrprog.eraseChip())
        {
          ...
          File f = SPIFFS.open(needUploadFilename, "r");
          if (f)
          {
            hexcode = (uint8_t*)malloc(f.size());
            if (hexcode)
            {
              int n = 0;
              while (f.available()) {
          ...
          if (hexcode)
          {
            if (avrprog.writeImage(hexcode, 512, 32768))
            {
    ...
    needUploadFilename = "";
#else
    msg = "Uploasd not allowed on this HUB model";
    DEBUGln(msg);
#endif
